...
public:
  ...
  //Merging
  void merge(std::shared_ptr<Node<T>>,
             std::shared_ptr<Node<T>>);
};
...
/**
Merge sorting of elements on list
 */
/*template <typename T>
void LinkedList<T>::sortMerge(){

  // List is empty
  if( isEmpty() )
    throw std::out_of_range("EmptyListException");

  //One node -> no need to sort 
  else if( size == 1)
    return;
...
/*template <typename T>
void LinkedList<T>::merge(std::shared_ptr<Node<T>> left,
